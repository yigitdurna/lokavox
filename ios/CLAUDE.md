...
1. `language = nil + detect_language = true` on `whisper_full` silently returns zero segments on this build/model. We sidestep that by doing what `whisper-cli` does for `-l auto`: a `whisper_pcm_to_mel` + `whisper_lang_auto_detect()` pre-pass before the full transcribe (`WhisperEngine.detectLanguage`). Costs ~100 ms per transcribe on Metal. The internal `detect_language = true` parameter on `whisper_full` itself stays off.
