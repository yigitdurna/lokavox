...
- **Custom vocabulary** and find-and-replace rules in Preferences
...
- **Accessibility** → required for global F5 capture and paste (`System Settings → Privacy & Security → Accessibility`)
...
- **Vocabulary** — one word or phrase per line, passed to whisper as a prompt so it recognizes domain terms
- **Corrections** — one per line, format `wrong >> right`, applied as word-boundary regex after transcription

Settings persist in `~/.local/share/lokavox/settings.json`.
...
- **Quartz CGEventTap** intercepts the dictation key globally and suppresses macOS's built-in dictation
...
| F5 still triggers macOS dictation | LokaVox not running or event tap disabled — restart the app |
